...
- 📈 **Real-Time Temp History Sparkline**: Ultra-smooth SVG area chart tracking temperature trends.
- ⚡ **Battery & Power Telemetry**: Real-time wattage draw (W), battery percentage, temperature, and cycle count via macOS `IOKit` (`IOPowerSources` & `AppleSmartBattery`).
...
| **Hardware FFI** | Apple SMC FFI (`smc.c`/`smc.h`), `IOKitLib`, `IOPowerSources`, `AppleSmartBattery` |
...
### 2. Build Production Application & DMG Bundle
```bash
...
For architectural design and migration details from SoloFan (Swift), refer to [CONVERSION_PLAN.md](CONVERSION_PLAN.md).
